...
import { fetchExpenses, selectExpensesData, selectExpenseStatus } from '../feature/expenses/expenseSlice'
...
const Expense = () => {
  ...
  const expenseStatus = useSelector(selectExpenseStatus);
  const expenseError = useSelector(selectExpenseStatus);

  useEffect(() => {
    if(page) dispatch(fetchExpenses({ page: page }));
  }, [page, dispatch]);

  const filteredExpense = useMemo(() => {
    const s = search.trim().toLowerCase();

    let lists = Array.isArray(expenses?.data) ? [...expenses?.data] : [];
        ...
        <div className='flex md:flex-row flex-col flex-1 justify-between md:items-center items-start'>
          <div className="md:space-y-2 space-y-1.5 md:my-2 my-1.5 md:p-2 p-0">
            ...
            <p className="md:text-base text-sm text-gray-500">
              Organize and manage your expenses.
            </p>
          </div>
          <Link to="/expenses/create" className='flex items-center md:gap-1.5 gap-1 text-white bg-blue-700 md:py-2.5 md:px-3 p-2 rounded-xl hover:bg-blue-500 transition-all ease-in-out duration-200 font-medium'>
            <FaPlus size={18}/>
            New Expense
          </Link>
        </div>
        ...
        <div className='md:mt-4 mt-8'>  
          ...
          <div className='md:px-2'>
            <div className="w-full overflow-x-auto border border-gray-200 bg-white">
              <table className="w-full">
                ...
                <tbody>
                  ...
                  {expenseStatus === "failed" && (
                    <tr>
                      <td colSpan={6}>
                        <ErrorMessage message="Failed to get expenses!" />
                      </td>
                    </tr>
                  )}
                  {
                    expenses?.data?.length > 0 && expenseStatus === "succeeded" && (
              ...
              {
                expenseStatus === "succeeded" && filteredExpense.length === 0 && (
